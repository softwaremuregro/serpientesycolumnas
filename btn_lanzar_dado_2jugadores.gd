...
var posicionJugador1: int = 0
var posicionJugador2: int = 0
...
func _on_pressed() -> void:
	...
	if turno_actual == 1:
		turno.text = nombre_j1+" Moviendo"		
		# Llamamos la función pasando a J1 y REASIGNAMOS el resultado a su variable
		posicionJugador1 = await mover_jugador(jugador1, posicionJugador1)
		# Una vez que terminó de moverse, cambiamos el turno al J2
		if posicionJugador1==posicionJugador2:
			jugador1.position = jugador1.position + Vector2(-20, 0)
			jugador2.position = jugador1.position + Vector2(80, 0)
		if posicionJugador1>=36:
			Global.nombre_jugador = nombre_j1 
			var escena_ganar2jugadores = preload("res://PANTALLAGANAR2jugadores.tscn")
			get_tree().change_scene_to_packed(escena_ganar2jugadores)
		mostrar_informacion2(posicionJugador1)
		turno_actual = 2
		turno.text = "Turno de "+nombre_j2
		turno.modulate=  Color8(101, 121, 119)
		turno.set("custom_colors/font_color", Color8(101, 121, 119)) # Gris
	elif turno_actual == 2:
		turno.text =  nombre_j2+" Moviendo"
		# Llamamos la misma función pero pasando los datos de J2
		posicionJugador2 = await mover_jugador(jugador2, posicionJugador2)
		# Cambiamos el turno de vuelta al J1
		mostrar_informacion2(posicionJugador2)
		if posicionJugador1==posicionJugador2:
			jugador1.position = jugador1.position + Vector2(-20, 0)
			jugador2.position = jugador1.position + Vector2(80, 0)
		if posicionJugador2>=36:
			Global.nombre_jugador = nombre_j2 
			var escena_ganar2jugadores = preload("res://PANTALLAGANAR2jugadores.tscn")
			get_tree().change_scene_to_packed(escena_ganar2jugadores)
		turno_actual = 1
		turno.text = "Turno de "+nombre_j1
		turno.modulate = Color8(244, 191, 0)
		turno.set("custom_colors/font_color", Color8(244, 191, 0)) # Amarillo
	pass
# LA FUNCIÓN GENÉRICA (Recibe el Sprite y la posición actual de quien tenga el turno)
func mover_jugador(jugador_actual: Sprite2D, posicion_actual: int) -> int:
	var movimiento_extra = 0
	button.disabled = true  # Deshabilitamos el botón mientras se mueve
	
	var puntuacion_dado = randi_range(1, 6)
	var suma = puntuacion_dado + posicion_actual
	
	mi_sprite.play(str(puntuacion_dado))
	
	# Bucle para el movimiento animado casilla por casilla
	for n in range(posicion_actual + 1, suma + 1):
		var marcador = nodo_camino.get_node_or_null(str(n))
		await get_tree().create_timer(1).timeout
		
		if marcador:
			# Aquí usamos el parámetro genérico 'jugador_actual'
			jugador_actual.position = marcador.position

	# --- Casillas especiales jugador ---

	if suma == 3:   movimiento_extra = 15
	elif suma == 7:  movimiento_extra = 32
	elif suma == 16: movimiento_extra = 27
	elif suma == 23: movimiento_extra = 26
	elif suma == 35: movimiento_extra = 5
	elif suma == 34: movimiento_extra = 22
	elif suma == 19: movimiento_extra = 8
	elif suma == 14: movimiento_extra = 11

	# Procesar si cayó en casilla especial o normal
	if movimiento_extra > 0:
		...
		button.disabled = false # Reincorporamos el botón al terminar
		return movimiento_extra # Devolvemos la casilla especial
	else:
		button.disabled = false # Reincorporamos el botón al terminar
		return suma # Devolvemos la casilla normal
